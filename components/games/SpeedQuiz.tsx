...
import { buildQuizQuestion, GameQuestion } from '@/lib/gameEngine'
...
export default function SpeedQuiz({ onExit }: SpeedQuizProps) {
  const pool = getAllVocab()
  const [question, setQuestion] = useState<GameQuestion | null>(() => buildQuizQuestion(pool))
  ...
  const answer = useCallback(
    (index: number | null) => {
      ...
      setTimeout(() => {
        ...
        setQuestion(buildQuizQuestion(pool))
        setSelected(null)
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 px-4">
      <div className="w-full max-w-md flex items-center justify-between text-sm text-gray-500">
        ...
        <span className="font-bold text-gold">⏱ {secondsLeft}</span>
        <span>
      ...
      <div className="card w-full max-w-md text-center py-8">
        <p className="text-xs text-gray-400 mb-2">מה פירוש המילה?</p>
        <p className="transliteration text-olive text-3xl font-bold">{question.word.transliteration}</p>
        <p className="text-xs text-gray-400 mt-1">{question.word.english_transliteration}</p>
      </div>
        ...
        {question.options.map((opt, i) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-3 rounded-xl text-right font-medium transition-colors ${style}`}
            >
              {opt}
            </button>
          )
