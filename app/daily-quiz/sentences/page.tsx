...
import { SentenceEntry, getAllSentences } from '@/lib/sentenceBank'
...
export default function DailySentenceQuizPage() {
  ...
  const [wrongSentences, setWrongSentences] = useState<SentenceEntry[]>([])
  ...
  useEffect(() => {
    async function build() {
      ...
      const failedSentences = Object.entries(statusMap)
        .filter(([, entry]) => entry.lastResult === 'wrong')
        .map(([id]) => allSentences.find((s) => s.id === id))
        .filter(Boolean) as SentenceEntry[]
      const dailySentences = getDailySentenceQueueWithLeftovers(
        settings.userLevel,
        settings.dailySentenceGoal ?? 5
      ).all

      const seen = new Set<string>()
      const combined: SentenceEntry[] = []
      for (const s of [...failedSentences, ...dailySentences]) {
        if (seen.has(s.id)) continue
        seen.add(s.id)
        combined.push(s)
      }
      const pool = combined.length > 0 ? combined : allSentences.slice(0, settings.dailySentenceGoal ?? 5)
      setQueue(pool.slice(0, settings.dailySentenceGoal ?? 5))
      setLoading(false)
    }
    build()
  }, [])

  const currentSentence = queue[index]
  const direction = directionFor(index)
  const question: SentenceGameQuestion | null = useMemo(
    () =>
      currentSentence
        ? buildQuestionForSentence(
            currentSentence,
            direction === 'transliteration-to-hebrew' ? 'hebrew_translation' : 'transliteration'
          )
        : null,
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [currentSentence?.id, direction]
  )

  const sentenceBreakdown = useMemo(() => {
    if (!currentSentence) return []
    return currentSentence.word_ids.map((id) => getVocabById(id)).filter(Boolean) as {
  ...
  const answer = useCallback(
    (optionIndex: number) => {
      if (selected !== null || !question || !currentSentence) return
      ...
      if (isCorrect) {
        ...
        setWrongSentences((w) => [...w, currentSentence])
        setStreak(0)
      }

      recordSentenceResult(currentSentence.id, isCorrect ? 'correct' : 'wrong')
    },
    [selected, question, currentSentence]
  )
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center pb-24 md:pb-0">
        <p className="text-gray-400">טוען בוחן משפטים יומי...</p>
        <BottomNav />
      </div>
    )
  }
  ...
  if (queue.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 pb-24 md:pb-0 px-4 text-center">
        ...
        <p className="font-bold text-olive">אין משפטים זמינים לבוחן היום</p>
        <BottomNav />
      </div>
    )
  }
    ...
    return (
      <div className="relative min-h-screen overflow-hidden pb-24 md:pb-0">
        ...
        <div className="relative z-10 flex flex-col items-center gap-6 py-16 px-4 text-center">
          ...
          >
            <h2 className="text-2xl font-bold text-olive-dark">סיימת את בוחן המשפטים היומי!</h2>
            ...
            <>
              ...
              <div className="w-full max-w-md text-right space-y-2">
                <p className="text-xs font-bold text-gray-500 px-1 flex items-center gap-1.5 justify-end">
                  <RotateCcw size={12} /> משפטים לחיזוק:
                </p>
                {wrongSentences.map((s) => (
                  <div key={s.id} className="card flex items-center justify-between py-2.5">
                    <div>
                      <p className="transliteration text-olive font-bold">{s.transliteration}</p>
                      <p className="text-xs text-gray-400 italic">{s.english_transliteration}</p>
                    </div>
                    <p className="text-sm text-gray-600">{s.hebrew_translation}</p>
                  </div>
                ))}
              </div>
            </>
  ...
  if (!question || !currentSentence) return null

  const promptText =
    direction === 'transliteration-to-hebrew' ? currentSentence.transliteration : currentSentence.hebrew_translation
  const promptHint = direction === 'transliteration-to-hebrew' ? 'מה פירוש המשפט?' : 'איך אומרים בתעתיק?'
  const score = masteredCount * 10
      ...
      <div className="relative z-10 flex flex-col items-center gap-4 px-4 pt-10 pb-8 safe-top md:pt-8">
        <div className="w-full max-w-md">
          <div className="mb-3 text-center md:text-right">
            <h1 className="text-lg font-bold text-olive-dark">📋 בוחן משפטים יומי</h1>
          </div>
          ...
          >
            <div className="text-center">
              ...
              {direction === 'transliteration-to-hebrew' ? (
                <>
                  <p className="transliteration text-xl font-bold text-olive-dark">{promptText}</p>
                  <p className="mt-1 text-xs text-gray-400">{currentSentence.english_transliteration}</p>
                </>
              ) : (
              ...
              >
                <AudioPlayer text={currentSentence.arabic_script} size="md" />
              </motion.div>
            </div>
        ...
        <AnimatePresence>
          {selected !== null && sentenceBreakdown.length > 0 && (
            <motion.div
            ...
            >
              ...
              {sentenceBreakdown.map((item, i) => (
                <div key={i} className="flex flex-wrap items-baseline gap-x-1.5 text-sm">
                  <span className="transliteration text-olive font-bold">{item.transliteration}</span>
                  <span className="text-gray-400">–</span>
                  <span className="text-gray-600">{item.hebrew_translation}</span>
                </div>
              ))}
            </motion.div>
          )}
